...
export const ProgramEditor = () => {
  ...
  obs('saveProg$', s => publish({ message: `Program ${s.programName} saved` }))
  obs('deleteProg$', s => publish({ message: `Program ${s.programName} deleted` }))

  const create = () => {
    if (!programName || !service || !changedProg) {
      return
    }
    const name = programName.endsWith('.ngc') ? programName : `${programName}.ngc`
    ...
    <div className={cardStretch} style={{ maxHeight: '100%', width: 560 }}>
      <div className={header} style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'inline-block' }}>
          {newProg
            ? <Input value={programName || ''} onChanged={setProgramName} width={20}/>
      ...
      <div className={content} style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        <div style={{ flex: '1', width: '100%' }} >
          <Editor style={{ flex: '1' }} file={program === undefined ? 'Select or create a program' : program } diff={undefined} onChanged={setChangedProg}/>
        </div>
