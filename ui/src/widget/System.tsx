...
export const System = () => {
  ...
  obs('systemSaved$', res => publish({ message: res.ok ? 'Settings are applied. Please restart the System' : 'Apply settings failed' }))

  const reload = () => service?.sendCommand({ cmd: 'settings', action: 'getSystem' })
  const save = () => settings && service?.sendCommand({ cmd: 'settings', action: 'setSystem', ...settings })
  ...
  return (
    <div className={cardStretch} style={{ width: 1600 }}>
      <div className={header}>
        System Settings <span style={{ color: 'red', fontSize: '20' }}>(requires restart)</span>
      </div>
        ...
        {(settings && (<>
          ...
          <div style={{ marginTop: 25, width: 550 }}>
            <InputToggle
              ...
              defaultValue={16}
              onChanged={(value: number | undefined) => setSettings({
                ...settings,
...
export const MotorBox = ({ title, motor, onChange, defaultSettings }: MotorBoxProps) => {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 24, marginBottom: 25 }}>
        {title}
      </div>
      <InputField
